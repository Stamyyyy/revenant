...
function pollUsnJournal(handle, layout, journalId, cursorUsn, index) {
  ...
  for (const rec of records) {
    ...
    // Only carry over the last-known size if this is genuinely the same
    // file as before (same sequence number) — a differing seq means the
    // MFT slot was freed and reused for something else since we last saw
    // it, and the old size would be meaningless for the new occupant.
    const carrySize = existing && existing.seq === rec.seq ? existing.size : 0;
...
function search(index, query, driveLetter, limit = 200) {
  ...
  for (const [recordNum, rec] of index) {
    ...
    if (!path) continue;
    results.push({ recordNum, fileId: fileId(recordNum, rec.seq), path, isDirectory: rec.isDirectory, size: rec.size });
...
function resultsForFileIds(index, fileIds, driveLetter, limit = 200) {
  ...
  for (const id of fileIds) {
    ...
    const path = resolvePath(index, recordNum, driveLetter);
    if (!path) continue;
    results.push({ recordNum, fileId: id, path, isDirectory: rec.isDirectory, size: rec.size });
    if (results.length >= limit) break;
  }
