...
function readAligned(fd, offset, length) {
  // Raw volume reads must land on sector-aligned offsets/lengths or Windows
  // returns EINVAL — every call site here already deals in sector/cluster
  // multiples, so this is just documentation, not a fixup.
  const buf = Buffer.alloc(length);
  const bytesRead = fs.readSync(fd, buf, 0, length, offset);
  if (bytesRead !== length) throw new Error(`short read at ${offset}: got ${bytesRead}/${length}`);
  return buf;
}
...
function readVolumeGeometry(fd) {
  const boot = readAligned(fd, 0, 512);
...
// Reads and returns { recordNum: {name, parentRecordNum, isDirectory, size} }
// for every in-use record on the volume. Progress is reported via onProgress
// (optional) as (recordsScanned, totalRecords).
function buildIndex(driveLetter, onProgress) {
  const fd = fs.openSync(`\\\\.\\${driveLetter}:`, 'r');
  try {
    const geo = readVolumeGeometry(fd);
    const { recordSize, mftOffset, clusterSize } = geo;

    // Record 0 is $MFT itself. Its unnamed $DATA attribute is non-resident
    // (the table is always far bigger than one record) and tells us both the
    // MFT's total length (real size) and, via its data runs, exactly which
    // clusters on disk actually hold it — which is NOT just "starting at
    // mftOffset and contiguous from there" on any volume that's seen use.
    const rec0 = readAligned(fd, Number(mftOffset), recordSize);
    ...
    const totalRecords = Math.floor(mftByteLength / recordSize);
    ...
    for (const run of runs) {
      ...
      while (streamPos < scanEnd) {
        ...
        const chunk = readAligned(fd, Number(physOffset), recordsThisRead * recordSize);
...
module.exports = { buildIndex, resolvePath, search };
