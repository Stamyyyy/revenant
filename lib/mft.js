...
//
// Returns null (rather than a garbage number) for anything outside a sane
// range. A corrupted/damaged record can carry an arbitrary 64-bit value
// here — without this guard, a bogus far-future timestamp would sort to the
// very top of the Recent list and stay pinned there forever (recentFiles
// sorts descending on mtime), silently hiding real recent files behind a
// corrupted one that can never be displaced. null is treated the same as
// "no $STANDARD_INFORMATION mtime" everywhere else in this file — the
// record is still indexed and searchable, it's just excluded from Recent.
function filetimeToMs(filetime) {
  const ms = Number(filetime / 10000n) - 11644473600000;
  if (ms < 0 || ms > Date.now() + 24 * 60 * 60 * 1000) return null;
  return ms;
}
...
function scanIndex(handle, layout, onProgress) {
  ...
  for (const run of runs) {
    ...
    while (streamPos < scanEnd) {
      ...
      for (let i = 0; i < recordsThisRead; i++) {
        ...
        // parseRecord trusts on-disk offset fields (contentOffset etc.) and
        // reads at them unguarded; a damaged/corrupted record can carry an
        // out-of-range offset that throws RangeError. Without this guard,
        // one bad record among millions would abort the entire index build
        // instead of just being skipped.
        let parsed;
        try {
          parsed = parseRecord(recBuf, recordSize);
        } catch (e) {
          corruptRecords++;
          continue;
        }
  ...
  if (corruptRecords > 0) {
    console.warn(`scanIndex: skipped ${corruptRecords} corrupted MFT record(s)`);
  }
  return { index, childrenIndex, corruptRecords };
}
...
function buildIndex(driveLetter, onProgress) {
  ...
  try {
    ...
    const { index, childrenIndex, corruptRecords } = scanIndex(handle, layout, onProgress);
    return { index, childrenIndex, totalRecords: layout.totalRecords, corruptRecords };
  } finally {
